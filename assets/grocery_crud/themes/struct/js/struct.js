$(function(){
	$('.quickSearchButton').click(function(){
		$(this).closest('.struct').find('.quickSearchBox').slideToggle('normal');
	});

	$('.ptogtitle').click(function(){
		if ($(this).hasClass('vsble')) {
			$(this).removeClass('vsble');
			$(this).closest('.struct').find('.main-table-box').slideDown("slow");
		} else {
			$(this).addClass('vsble');
			$(this).closest('.struct').find('.main-table-box').slideUp("slow");
		}
	});
	...
	$('.filtering_form').submit(function(){
		var crud_page =  parseInt($(this).closest('.struct').find('.crud_page').val(), 10);
		var last_page = parseInt($(this).closest('.struct').find('.last-page-number').html(), 10);

		if (crud_page > last_page) {
			$(this).closest('.struct').find('.crud_page').val(last_page);
		}
		if (crud_page <= 0) {
			$(this).closest('.struct').find('.crud_page').val('1');
		}
		...
		$(this).ajaxSubmit({
			 ...
			 dataType: 'json',
			 beforeSend: function(){
				 this_form.closest('.struct').find('.ajax_refresh_and_loading').addClass('loading');
			 },
			 complete: function(){
				 this_form.closest('.struct').find('.ajax_refresh_and_loading').removeClass('loading');
			 },
			 success:    function(data){
				this_form.closest('.struct').find('.total_items').html( data.total_results);
				displaying_and_pages(this_form.closest('.struct'));

				this_form.ajaxSubmit({
					 success:    function(data){
						this_form.closest('.struct').find('.ajax_list').html(data);
						call_fancybox();
						add_edit_button_listener();
					 }
				});
		...
		if ($('.struct').length == 1) { //disable cookie storing for multiple grids in one page
			...
			createCookie('search_text_'+unique_hash,$(this).closest('.struct').find('.search_text').val(),1);
	...
	$('.crud_search').click(function(){
		$(this).closest('.struct').find('.crud_page').val('1');
		$(this).closest('.struct').find('.filtering_form').trigger('submit');
	});

	$('.search_clear').click(function(){
		$(this).closest('.struct').find('.crud_page').val('1');
		$(this).closest('.struct').find('.search_text').val('');
		$(this).closest('.struct').find('.filtering_form').trigger('submit');
	});
	...
	$('.prev-button').click(function(){
		if( $(this).closest('.struct').find('.crud_page').val() != "1")
		{
			$(this).closest('.struct').find('.crud_page').val( parseInt($(this).closest('.struct').find('.crud_page').val(),10) - 1 );
			$(this).closest('.struct').find('.crud_page').trigger('change');
		}
	});

	$('.last-button').click(function(){
		$(this).closest('.struct').find('.crud_page').val( $(this).closest('.struct').find('.last-page-number').html());
		$(this).closest('.struct').find('.filtering_form').trigger('submit');
	});

	$('.next-button').click(function(){
		$(this).closest('.struct').find('.crud_page').val( parseInt($(this).closest('.struct').find('.crud_page').val()) + 1 );
		$(this).closest('.struct').find('.crud_page').trigger('change');
	});
	...
	$('.ajax_list').on('click','.field-sorting', function(){
		$(this).closest('.struct').find('.hidden-sorting').val($(this).attr('rel'));

		if ($(this).hasClass('asc')) {
			$(this).closest('.struct').find('.hidden-ordering').val('desc');
		} else {
			$(this).closest('.struct').find('.hidden-ordering').val('asc');
		}
	...
	$('.ajax_list').on('click','.delete-row', function(){
		...
		var this_container = $(this).closest('.struct');
	...
	$('.export-anchor').click(function(){
		...
		var form_input_html = '';
		$.each($(this).closest('.struct').find('.filtering_form').serializeArray(), function(i, field) {
		...
		$(this).closest('.struct').find('.hidden-operations').html(form_on_demand);

		$(this).closest('.struct').find('.hidden-operations').find('#export_form').submit();
	});
	...
	$('.print-anchor').click(function(){
		...
		$(this).closest('.struct').find('#print_form').ajaxSubmit({
			beforeSend: function(){
