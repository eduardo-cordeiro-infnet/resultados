				...
				if(data.success)
				{
					$('#crudForm').ajaxSubmit({
						...
						success: function(result){
							...
							if(data.success)
							{
								var data_unique_hash = my_crud_form.closest(".struct").attr("data-unique-hash");

								$('.struct[data-unique-hash='+data_unique_hash+']').find('.ajax_refresh_and_loading').trigger('click');
